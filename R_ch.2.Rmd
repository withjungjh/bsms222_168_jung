---
title: "Assignment_ch2.2-2.6+2.7~"
author: "Jung, Jin Hwan"
output: html_notebook
---
  
  
# 2.2 __THe very basics__  
...
We should see `a`,`b`, and `c`. If you try to  recover the value of a variable that is not in your workspace, receive an error. For example, if you type `x` you will receive the following message: `Error: object 'x' not found`
...
## __2.2.4 Other prebuilt objects__  
...
## __2.2.6 Saving your workspace__  
...
## __2.2.8 Commenting your code__  


If a line of R code with the symbol `#`, it is not evaluated. We can use this to write reminders of why we wrote particular code. For example, in the script above we could add:
...
str(murders)
```
This tells us much more about the object. We see that the table has 51 rows (50 states plus DC) and five variables. We can show the first six lines using the fuction `head`:
```{r}
head(murders)
```
...
__Tip__: R comes with a very nice auto-complete functionality that saves us the trouble of typing out all the names. Try typing `murders$p` then hitting the _tab_ key on your keyboard. This functionality and many other useful auto-complete features are available when working in RStudio.


## __2.4.4 Vectors: numerics, characters, and logical__  
...
To store character strings, vectors can also be a class character. For example, the state names are characters:
```{r}
class(murders$state)
```
As with numeric vectors, all entires in a character vector need to be a character.

Another important type of vectors are _logical vectors_. These must be either `TRIE` or `FALSE`.
```{r}
z <- 3==2
z
class(z)
```
Here the `==` is a relation operator asking if 3 is equal to 2. In R, if you just use one `=`, you actually assign a variable, but if you ust two`==`, you test for equality.

You can see the other _relation operators_ by typing:
```{r}
?Comparison
```

In furture section, you will see how useful relational operators can be.

We discuss more important features of vectors after the next set fo exercises.

__Advanced__:Mathematically, the values in `pop` are integers and there is an integer class in R. However, by default, numbers are assigned class numeric even when they are round integers. For example, `class(1)` returns numeric. You can turn them into class integer with the `as.integer()` function or by adding an `L` like this:`1L`. Note the class by typing: `class(1L)`
```{r}
class(1L)
```
...
```
It is _factor_. Factors are useful for storing categorical data. We can see that there are only 4 regions by using the `levels` function:
```{r}
levels(murders$region)
```
IN the background, R stores these _levels_ as integers and keeps a map to keep trakc the labels. This is more memory efficient than storing all the characters.

Note that the levels have an order that is different form the order of appearance in the factor object. The default in R is for the levels to follow alphabetical order. However, often we want the levels to follow a different order. You can specify an order through the `levels` argument when creating the factor with the `factor` function. For example, in the murders dataset regipons are ordered from east to west. The function `reorder` lets us change the order of the levels of a factor variable based on a summary computed on a numeric vector. We will demonstrate this with a simple example, and will see more advanced ones in the Data Visualization part of the book.

Suppose we want the levels of the region by the total number of murders rather than alphabetical order. If there are values associated with each level, we can use the `reorder` and specify a data summary to determine the order. The following codei takes the sum of the total murders in each region, and rerorder the factor following these sums.
```{r}
...
__Warning__: Factors can be a source of confusion since sometimes they behave like characters and sometimes they do not. As a result, confusing factors and characters are a common source of bugs.
...
record2[[2]]
```
We won't be using lists until later, but you might encounter one in your own exploration of R. For this reason, we show you some basic here.
...
mat[2,]
```
Notice that this return a vector, not a matrix.
...
You can access more than one columnn or more than row if you like. This will give you a new matrix.
```{r}
mat[,2:3]
```
...
as.data.frame(mat)
```
You can also single square brackets(`[`) to access rows and columns of a data frame:
```{r}
data("murders")
...
## __2.6.1 Creating vectors__  


Wecan create vectors using the function `c`, which stands for _concatenate_. We use `c` to concatenate entries in the following way:
```{r}
codes <- c(380, 124, 818)
codes
```
...
seq(1,10)
```
The first argument defines the start, and the second defines the end which is included. The default is to go up in increments of 1, but the third argument lets us tell it how much to jump by:
```{r}
seq(1,10,2)
```
...
1:10
```
When we use these funtions, R produes integers, not numerics, because they are typically used to index something:
```{r}
class(1:10)
```
...
codes[1:2]
```
If the eleements have names, we can also access the entries using these names, Below are two examples.
```{r}
codes["canada"]
codes[c("egypt", "italy")]
```
...
```{r}
x
order(x) # vector에 원래 붙어있던 index를 increasing order로 배열한 것
```
...
## __2.9.5 Beware of recycling__

Another common source of unnoticed errors in R is the use of _recycling._ We saw that vectors are added elementwise. So if the vectors don’t match in length, it is natural to assume that we should get an error. But we don’t. Notice what happens:
```{r}
x <- c(1,2,3)
...
i_pop <- which.min(murders$population)
murders$population[i_pop]
```
...
2. What is the following sum $1 + 1/2^2 + 1/3^2 +...+1/100^2$? Hint: thnaks to Euler, we know it should be close to $\pi^2/6$.
...
__Advanced__: There is a connection between `match` and `%in%` through `which`. To see this, notice that the following two lines produce the same index (although in different order):
```{r}
match(c("New York", "Florida", "Texas"), murders$state) # match one character and do other things 
which(murders$state %in% c("New York", "Florida", "Texas")) # match whether there is thing amoung the given vecter in order of murders$state
```
...
average
bel_mean <- which(murder_rate < average)
length(bel_mean)
```
...
```{r}
ind_add <- match(c("AK", "MI", "IA"), murders$abb) # Don't forget to consider using "" in character
murders$state[ind_add]
```
...
```{r}
right_abb <- which(murders$abb %in% c( "MA", "ME", "MI", "MO", "MU"))
murders$abb[right_abb]
```

8. Extend the code you used in exercise 7 to report the one entry that is __not__ an actual abbreviation. Hint: use the `!` operator, which turns `FALSE` into `TRUE` and vice versa, then `which` to obtain an index.
```{r}
m <-  c( "MA", "ME", "MI", "MO", "MU")
wrong_abb <- which(!m %in% murders$abb)
m[wrong_abb] 
```

# __2.15 Basic plots__
...
```{r}
murders$rate <- with(murders, total / population * 100000) # murders$state <- 여기에 새로운 state라는 column을 추가한다는 의미
...
```{r}
population_in_millions <- murders$population/10^6
total_gun_murders_log <- log(murders$total, base=10)
plot(population_in_millions, total_gun_murders_log)
```

2. Create a histogram of the state population
```{r}
x <- with(murders, population)
hist(x)
```

3. Generate boxplot of the state population by region.
```{r}
